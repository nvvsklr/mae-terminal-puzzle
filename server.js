...
const RiriOrderTracker = require('./src/mae-order-tracker');
...
// Initialize Database and Riri Order Tracker
const database = new DatabaseManager();
const ririTracker = new RiriOrderTracker(database);
...
io.on('connection', async (socket) => {
  ...
  socket.emit('riri-response', {
    message: "hey there! (⁄ ⁄>⁄ ▽ ⁄<⁄) i'm riri, your digital order tracking assistant!\n\nplease enter your email address to find your order tracking information\nexample: yourname@email.com",
    type: 'riri',
    delay: 1000
  });
  ...
  socket.on('user-message', async (data) => {
    ...
    try {
      ...
      // Handle order tracking conversation
      console.log('📦 Handling order tracking request for session:', session.id);
      const ririResponse = await ririTracker.getRiriResponse(data.message, session, socket);
      
      // For non-async responses, send immediately
      if (ririResponse) {
        response = {
          message: ririResponse,
          type: 'riri'
        };

        // Show typing indicator before response
        socket.emit('riri-typing', { typing: true });
        
        // Simulate Riri thinking time (800ms to 2000ms)
        const typingDelay = Math.random() * 1200 + 800;
        
        setTimeout(() => {
          socket.emit('riri-typing', { typing: false });
          socket.emit('riri-response', response);
        }, typingDelay);
      }
      ...
      socket.emit('riri-typing', { typing: false });
      socket.emit('riri-response', {
        message: "system error... please try again (〃ﾟ3ﾟ〃)",
        type: 'error'
      });
    }
  });
